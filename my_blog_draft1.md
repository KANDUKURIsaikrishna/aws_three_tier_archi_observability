# Implementing a higly avalable higly scalble resilent micro service based application

What actually breaks when you split a monolith into microservices? I found out the expensive way. I took a working three-tier bookstore  one React frontend, one Node/Express backend, one MySQL database and rebuilt the backend as five independent services on Amazon EKS (AWS's managed Kubernetes). Infrastructure in Terraform. Delivery through ArgoCD. A CI/CD pipeline that can stop a release at five separate points. Then I spent most of the time chasing the failures that only turn up once the thing is running and charging you.

About 2,000 lines of Terraform. Five pipeline checkpoints. Observability was the piece that didn't work the first time; the in-cluster design failed outright and got rebuilt on its own EC2 instance. Disaster recovery got built, then audited, and the audit mostly told me how much less it covered than I'd assumed.

None of this came from a production incident. It came from the gap between reading how distributed systems fail and running one yourself, on infrastructure you own, with a bill attached, tracing each failure back from `kubectl get pods`.

The post is long. Jump around. If you read two sections, make them **Monitoring vs. the cluster's own resource limits** and **the External Secrets bug**. Both looked healthy while they were broken, long enough that the debugging is the useful part.
...
**Entry point:** Route53, active-passive failover routing. CloudFront is provisioned but off by default. Not worth it at this traffic.
...
The real service map, read off the code rather than from memory:
...
I read the file top to bottom and assumed that's the build order. It isn't. **Where a resource sits in a `.tf` file means nothing to Terraform.** It builds the dependency graph, then runs everything that isn't blocked, in parallel, wherever it happens to be in the file.

My RDS module and my EKS module never reference each other. Both hang off the network layer and nothing else. RDS is written first. Terraform brings both up at the same time.

- Database: ~10–15 min
- Cluster: ~15–20 min
- Full stand-up: roughly the slower of the two, not the sum

Provisioning worked. Teardown didn't. AWS and Kubernetes both create things as side effects (ENIs, security groups, load balancers) that Terraform never wrote into its state. So `terraform destroy` walks up to a VPC that still has an untracked network interface attached, tries to delete it, and dies with `DependencyViolation` and no hint about which dependency.
...
The lesson: a chunk of what the cloud provider creates for you never lands in the tool that's supposed to be tracking your infrastructure. You clean it up yourself, on purpose, or `destroy` hangs.
...
**Root cause:** one `t3.medium` node. I'd been thinking about CPU and memory. Wrong axis. AWS caps pods per instance on ENI count (the virtual NICs each pod pulls its IP from), not CPU or memory, and a `t3.medium` tops out at 17 pods no matter how idle it is.
...
**Fix:** not a bigger node. Take the monitoring stack out of Kubernetes scheduling entirely. Prometheus, Grafana, and Loki now sit on a dedicated `t3.small` behind Docker Compose, where the scheduler has no say.
...
Two host-level jobs run outside the monitoring box, on the EKS worker nodes themselves. The EKS module's node launch template (`node-user-data.sh.tftpl`) installs them, not the monitoring instance's boot script. `node-exporter v1.8.2` and Fluent Bit run as plain systemd services on each node. Not DaemonSets. The whole point was zero monitoring pods inside the cluster. Fluent Bit has no static log endpoint to aim at, so at boot it finds the monitoring EC2's private IP itself with `aws ec2 describe-instances` (retried, because the node's IAM permission for that call can lose the same propagation race as below). The box's public Elastic IP doesn't route back cleanly from a private subnet in the same VPC, that is why it needs the private one.
...
**1. Install happens through EC2 user data, not an SSH session.** Terraform hands the instance a boot script. AWS calls this "user data": code the instance runs once, automatically, on first boot. The script pulls Docker and the Compose plugin from Docker's own apt repo (Ubuntu's don't carry the plugin), installs `kubectl` v1.31.0, then fetches three secrets from AWS Secrets Manager: the Grafana admin password, a shared basic-auth credential for Prometheus and Alertmanager, and the SES SMTP credentials. Neither Prometheus nor Alertmanager ships with a login, so without it, anyone in the allowed IP range can open either UI and silence a firing alert through Alertmanager's API. Each fetch retries 20 times, 6 seconds apart. That's on purpose. The IAM policy that lets the instance read those secrets gets created in the same `terraform apply` as the instance, and IAM isn't instantly consistent. First boot can lose the race.

**2. Two cron jobs keep things current after boot.** One refreshes the Kubernetes access token every 10 minutes and restarts `kube-state-metrics` each time, because that container only reads its kubeconfig at startup and a stale token would otherwise stick forever. The other rewrites Prometheus's scrape-target files every 5 minutes off `aws ec2 describe-instances`, so nodes joining or leaving get picked up without a Prometheus restart.

**3. Dashboards land two different ways.** Which one matters the day you need to change one. Exactly one dashboard is pulled live: "Node Exporter Full", a well-known community dashboard (Grafana.com ID `1860`, revision `37`, pinned), downloaded and `POST`ed into Grafana through its import API, with the datasource pointer rewritten to the local Prometheus. The other two, a custom pod/node resource view and a cluster overview, are hand-built JSON checked into the repo. They reach Grafana through file provisioning: dropped into a folder Grafana scans on startup. No API call, no download, no dashboard ID. I tried a third community dashboard, ID `315`, and dropped it. It queries a label this setup's metrics don't produce. The cluster-overview one I wrote is the replacement.
...
One thing that's easy to miss underneath all this: SES SMTP auth won't take the IAM access key's raw secret. It wants a *derived* SMTP password, run through AWS's documented HMAC-SHA256 conversion. Terraform has no function for that, so the derivation is a small Python script called from `local-exec`, and the output goes straight into Secrets Manager. The derived password never touches Terraform state.


Would I build it this way on a 50-node cluster? No. This is a fix for one `t3.medium` with a hard pod ceiling, nothing more. What carries over is smaller: if your monitoring is starving the app of the resources the app needs to run, move the monitoring. Don't shrink the app to fit it next to Prometheus.
...
**Deploy model:** ArgoCD. GitOps means the git repo is the one place that says what should be running, and a controller in the cluster keeps reality matching it. CI never runs `kubectl` and never holds cluster credentials. It authenticates to AWS with OIDC, a short-lived token swap, so there are no static AWS keys sitting in GitHub Secrets waiting to leak.
...
**Failure mode:** build-and-scan takes about 15 minutes. So by the time the deploy step goes to commit, its checkout of the branch is already 15 minutes old. Push two builds close together and the second `git push` bounces. The branch moved under it.
...
The pods that needed those secrets looked like they had a boring database connection problem. So that's what I debugged. I checked the RDS security group, the connection string, the DB user's grants, whether the subnet routes were even right. A couple of hours. Then it occurred to me the Secret itself was empty.
...
That's what "no message queue" actually looks like in code. Checkout never waits on `notification-service`. The client has its `201` before the notify call even fires, and if that call times out, nothing rolls back and nobody's told. A queue wouldn't make this faster. It'd make the dropped notification recoverable.
...
The `count` there is deliberate. Replication stays off unless you pass a customer-managed KMS key ARN, because AWS's default keys don't work across regions. I made it opt-in after an earlier version quietly turned on cross-region replication on every single apply, whether anyone asked for it or not. That one showed up on the bill.

**On by default:** `us-west-2` holds an ECR replica and an automated RDS backup replica. Nothing else. No cluster, no running app. Lose `us-west-1` and the data's safe — but nothing serves a request until you restore it.

**Opt-in, behind `enable_dr_standby`:** the rest. A second EKS cluster and add-ons. A promotable RDS cross-region read replica. Cross-region copies of every secret the cluster reads. The Route53 failover record is already there — it serves the second the primary health check drops. Promoting the database is a manual step on purpose. You can't un-promote a replica, so you don't want that automated and firing on a health-check flap.

It roughly doubles the bill. So it's off unless you turn it on. That's the real line between backup and failover: the switch exists, and whether the second bill is worth it isn't a default I get to make for you.
...
If you clone this and something doesn't line up with what I've written, or you'd have made a different call somewhere, I want to hear it.
